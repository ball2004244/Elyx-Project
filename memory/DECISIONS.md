...
| D26 | Routine display | Single "Daily Protocol" panel (option b), shown once — NOT repeated per day column | A longevity daily protocol is largely constant Mon-Sun; showing 53 routines once (grouped by type) + per-day events on the calendar is far more readable than 53 rows x 7 days. |
| D27 | Calendar shows events only | Week/Day calendar renders EVENTS (resource/venue-bound); routines live in the Daily Protocol panel | Events are what vary day to day and need slots; routines are checklist items. Splits the 90-card wall into ~26 event cards + 1 protocol panel. |
...
| D35 | Daily Protocol accordion | Collapsible-by-type, collapsed by default, summary chips | 59 rows at once is overwhelming; one summary line expanding on demand fits the taste-skill "long lists need a different component" rule. |
