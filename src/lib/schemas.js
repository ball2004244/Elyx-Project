 ...
 * Pydantic): each schema both validates data at runtime AND serves as the single
 * source of truth for the corresponding object shape.
 *
 * Terminology is locked in memory/GLOSSARY.md:
 *  - Action Plan  -> the INPUT (priority-ordered list of Activities)
 *  - 5 Constraints -> Client's Schedule, Travel Plans, Equipment, Specialists, Allied Health
 *  - 3 Forces      -> Member fit, Resource availability, Health efficacy
 *
 * Convention: resource pools (Equipment, Specialists, Allied Health) publish the
 * windows in which they ARE available. The member's Client Schedule and Travel
 * publish BLOCKED windows (commitments / time away).
 */
 ...
 *  - alliedHealth -> Constraint C5 (physio, OT, dietitian, speech therapist, trainer)
 */
...
export const Facilitator = z.object({
  ...
  /** Role label, e.g. "trainer", "physiotherapist", "physician", "dietitian". */
  role: z.string().min(1).optional(),
  /** Optional pin to a specific resource id from the Specialists/AlliedHealth pool. */
  resourceId: z.string().min(1).optional(),
});

/**
 * Activity = one action in the Action Plan. Carries the 10 assignment fields
 * plus `id` and `priority`.
 *
 * NOTE (extension): `requiredEquipment` is added beyond the assignment's 10
 * fields so the Equipment constraint (C3) can actually be checked during
 * scheduling. It is optional and defaults to none.
 */
export const Activity = z.object({
  /** Stable identifier. */
  id: z.string().min(1),
  /** Priority = health importance; lower number = higher priority (1 is top). */
  ...
  // (7) Prep needed to facilitate the activity, e.g. "food needs to be cooked".
  prep: z.string().default(''),
  // (8) Backup activities that can substitute for this activity (ids or labels).
  ...
  /** Extension: equipment ids this activity needs (Constraint C3). */
...
/** C2 - Travel Plans: BLOCKED windows when the member is away / location changes. */
...
/** C3 - Equipment: a physical item and the windows in which it IS available. */
...
export const Specialist = z.object({
  ...
  name: z.string().min(1),
  role: z.string().min(1),
  /** Can facilitate remotely (relaxes location/travel for remote activities). */
  remoteOk: z.boolean().default(false),
  availability: z.array(TimeWindow).default([]),
});

/** C5 - Allied Health: physio, OT, dietitian, speech therapist, trainer, etc. */
...
/** One concrete, time-boxed occurrence in the Personalized Plan (the OUTPUT). */
export const ScheduledInstance = z
  .object({
    ...
    metrics: z.array(z.string()).default([]),
    note: z.string().default(''), // e.g. skip-adjustment text or substitution reason
  });
